...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 27-09-2018 a las 04:29:08
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_HORARIO`(IN `Diapar` INT(1), IN `Hora_iniciopar` TIME, IN `Hora_finpar` TIME)
    NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_HORARIO`(IN `Diapar` INT(1), IN `Hora_iniciopar` TIME, IN `Hora_finpar` TIME, IN `idpar` INT)
    NO SQL
